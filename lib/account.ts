...
export async function checkSubscription(userId: string) {
  try {
    ...
    if (_subscriptions.length === 0) return false;
...
export const getFeatureFlags = async (userId: string) => {
  try {
    const flags = await db.select().from(feature_flags);
    return flags.reduce((acc, curr) => {
      acc[curr.flag as FeatureFlags] = curr.enabled;
      return acc;
    }, {} as any);
  } catch (err) {
...
export const getAppSettings = async (userId: string) => {
  try {
    const settings = await db.select().from(app_settings);
    return settings.reduce((acc, curr) => {
      acc[curr.name as AppSettings] = curr.value;
      return acc;
    }, {} as any);
  } catch (err) {
    logger.error("Error getting app settings", err);
    return null;
  }
};
